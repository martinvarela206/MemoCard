...
- **[src/utils/randomBalancer.js](../src/utils/randomBalancer.js)**: Algoritmo de navegación aleatoria circular con balanceo ponderado por sesión (ventana de 1 hora), muestreo por rechazo (`P = K / N`) y reducción de línea base para garantizar distribución uniforme de tarjetas.
...
- **[src/components/FlashCard.jsx](../src/components/FlashCard.jsx)**: Tarjeta interactiva con giro 3D, compilación matemática KaTeX en títulos (`card-term`, `card-mini-title`) y badges de subtema, área vertical maximizada (sin pie de tarjeta ni tags redundantes), soporte multimedia (`CardMedia`) y badges de secuencia de derivación por pasos (`Paso X de Y`).
...
- **[src/components/StudyDrawer.jsx](../src/components/StudyDrawer.jsx)**: Drawer lateral con acordeón temático, soporte KaTeX en títulos (`card-item-title`) y subtemas, barra de filtro transversal por tags (`#definicion`, `#examen`), insignias de subtema y secuencias de derivación.
